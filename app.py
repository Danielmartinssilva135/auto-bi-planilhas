...
# Mapeamento para ordenação cronológica de meses
ORDEM_MESES = [
    "Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho",
    "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"
]

# Função para leitura universal de planilhas e higienização de dados
def carregar_planilha(arquivo):
    ...
    # Remove linhas residuais de total da planilha de origem para não duplicar somas
    col_zero = df.columns[0]
    df = df[~df[col_zero].astype(str).str.strip().str.upper().isin(["TOTAL", "TOTAL GERAL", "SUBTOTAL"])]
    
    # Se a primeira coluna contiver nomes de meses, preserva a ordem cronológica
...
def exportar_excel_profissional(df, titulo="RELATÓRIO CONSOLIDADO"):
    ...
    # 3. Linhas com formatação zebrada
        ...
        try:
            ...
            if eixo_coluna == "Nenhum":
                if tipo_agregacao == "count":
                    pivot_table = df_calc.groupby(eixo_linha, observed=False)[[eixo_valor]].count().rename(columns={eixo_valor: f"Qtd de {eixo_valor}"})
                elif tipo_agregacao == "sum":
                    pivot_table = df_calc.groupby(eixo_linha, observed=False)[[eixo_valor]].sum().rename(columns={eixo_valor: f"Soma de {eixo_valor}"})
                elif tipo_agregacao == "mean":
                    pivot_table = df_calc.groupby(eixo_linha, observed=False)[[eixo_valor]].mean().rename(columns={eixo_valor: f"Média de {eixo_valor}"})
                elif tipo_agregacao == "min":
                    pivot_table = df_calc.groupby(eixo_linha, observed=False)[[eixo_valor]].min().rename(columns={eixo_valor: f"Mínimo de {eixo_valor}"})
                elif tipo_agregacao == "max":
                    pivot_table = df_calc.groupby(eixo_linha, observed=False)[[eixo_valor]].max().rename(columns={eixo_valor: f"Máximo de {eixo_valor}"})
            else:
            ...
            # Se a linha for mês, garante a ordenação cronológica na tabela dinâmica
            if str(pivot_table.index.name) in df_pivot.columns and df_pivot[pivot_table.index.name].astype(str).isin(ORDEM_MESES).any():
                meses_presentes = [m for m in ORDEM_MESES if m in pivot_table.index]
                if meses_presentes:
                    pivot_table = pivot_table.reindex(meses_presentes)
